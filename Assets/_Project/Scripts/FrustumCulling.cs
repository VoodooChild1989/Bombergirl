    ...
    #region METHODS
        ...
        private void CheckObjects()
        {
            ...
            for (int i = pooledObjects.Count - 1; i >= 0; i--)
            {
                ...
                if (!IsInFrustum(obj.transform.position))
                {
                    Destroy(obj);
                    pooledObjects.RemoveAt(i);
                    continue;
        ...
        private IEnumerator MapperCoroutine()
        {
            ...
            for (int i = 0; i < CA.width; i++)
            {
                for (int j = 0; j < CA.height; j++)
                {
                    ...
                    /// <summary>
                    /// The enemies are spawned in empty cells.
                    /// </summary>
                    if (CA._grid[i,j] == 0) 
                    {
                        // Adding enemies
                        if ((IsInFrustum(worldPos)) && (CA._isUsedGrid[i,j] == 0) && (i >= 1) && (i <= CA.width - 2) && (j >= 1) && (j <= CA.height - 2))
                        {
                            if (CA.IsValue(0.3f)) 
                            {   
